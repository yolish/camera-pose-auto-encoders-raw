...
class CameraPoseDatasetExt(CameraPoseDataset):
    """
        A class representing a dataset of images and their poses
    """

    def __init__(self, dataset_path, labels_file, base_transform, data_transform=None, equalize_scenes=False):
        """
        ...
        """
        super(CameraPoseDatasetExt, self).__init__(dataset_path, labels_file, data_transform, equalize_scenes)
    ...
    def __getitem__(self, idx):
        ...
        small_img = self.transform(img)
        if self.transform:
            img = self.transform(img)

        sample = {'img': img, 'pose': pose, 'scene': scene, 'small_img':small_img}
        return sample
